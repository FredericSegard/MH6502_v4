...
basic_interpreter:
	...
	beq	basic_prep		; End of buffer (0)? Then get default address
	cmp	#" "			; Is it the space delimiter?
	beq	basic_help		; Yes it is. Then check if its a help request, or an address
	jmp	syntax_error		; If anything else, print a syntax error, and end routine

basic_help:
	jsr	skip_spaces		; Skip any leading spaces
	lda	IN_BUFFER, y		; Read a character from the input buffer
	beq	basic_prep		; End of buffer? Then get default address
	cmp	#"?"			; Is help requested?
	bne	basic_prep		; No, then get address
	printString help_basic		; *MACRO*
